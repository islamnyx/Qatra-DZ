...
import { donor, getMostUrgentSOS } from "../mockData";
...
export default function Home() {
  const { t } = useLanguage();
  const { markResponded, hasResponded, showSuccess, clearSuccess } = useApp();
  const urgentSOS = getMostUrgentSOS();
  const matchScore = calculateMatchScore(urgentSOS);
  ...
  return (
    <div className="mx-auto min-h-screen max-w-sm bg-red-50 pb-24">
      ...
      <main className="space-y-4 px-4 py-4">
        <div className="animate-fade-in-up">
          <SOSBanner
            ...
            onShare={handleShare}
            responded={hasResponded(urgentSOS.id)}
          />
        </div>
        ...
        <section className="grid grid-cols-3 gap-2 animate-fade-in-up stagger-2">
          {[
            ...
            { icon: Award, value: donor.badges.length, label: t("badges") },
      ...
      <BottomNav />
      {modalOpen && (
        <SOSRespondModal request={urgentSOS} matchScore={matchScore} onClose={() => setModalOpen(false)} onConfirm={() => { markResponded(urgentSOS.id); setModalOpen(false); }} />
      )}
