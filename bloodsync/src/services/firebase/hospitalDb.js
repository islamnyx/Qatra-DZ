...
import { COLLECTIONS } from "@shared/hospital-db/schema.js";
import {
  buildInventoryUnit,
...
export async function listBloodInventory(filters = {}) {
  ...
  if (constraints.length) constraints.push(orderBy("expiration_date", "asc"));
...
export async function getBloodUnit(unitId) {
  const db = dbOrThrow();
  const snap = await getDoc(doc(db, COLLECTIONS.BLOOD_INVENTORY, unitId));
...
export async function deleteBloodUnit(unitId) {
  await deleteDoc(doc(dbOrThrow(), COLLECTIONS.BLOOD_INVENTORY, unitId));
}
...
export async function getHospitalsFromInventory() {
  return aggregateInventoryByHospital(await listBloodInventory());
}
  ...
  return units
    ...
    .map((u) => ({
      ...
      hoursRemaining: Math.max(
        0,
        Math.floor((new Date(`${u.expiration_date}T23:59:59Z`).getTime() - Date.now()) / 3600000)
      ),
    }));
}
...
export async function getInventoryKpis() {
  ...
  return {
    ...
    expiringThisWeek,
    activeShortages: pending.filter((r) => r.urgency_level === URGENCY_LEVEL.CRITICAL).length,
    transfersToday: 0,
  };
}
